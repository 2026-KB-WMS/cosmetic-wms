...
import com.kb.cosmetic_wms.oms.adapter.out.external.WarehouseCandidateAdapter;
...
class WarehouseCandidateAdapterTest {
    ...
    @Mock
    private FindProductAvailabilityUseCase findProductAvailabilityUseCase;
    ...
    void 전체_창고를_후보로_로드하고_창고별_상품_재고_요약을_매핑한다() {
        ...
        given(findWarehouseUseCase.findAll()).willReturn(List.of(
                warehouseResult(1L, "37.2410000", "127.1775000"),
                warehouseResult(2L, "35.1796000", "129.0756000")
        ));
        given(findProductAvailabilityUseCase.findAvailabilityByProducts(anyCollection())).willReturn(List.of(
                new ProductAvailabilitySlice(1L, 100L, 30, expiryA),
                new ProductAvailabilitySlice(1L, 200L, 5, expiryB),
                new ProductAvailabilitySlice(2L, 100L, 50, expiryB)
        ));
    ...
    private static WarehouseResult warehouseResult(Long id, String lat, String lng) {
        return new WarehouseResult(id, "창고" + id, "주소" + id,
                new BigDecimal(lat), new BigDecimal(lng), "10~25도", 10000, List.of());
    }
}
