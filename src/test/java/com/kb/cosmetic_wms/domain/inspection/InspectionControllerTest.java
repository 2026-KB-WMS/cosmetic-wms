...
import com.kb.cosmetic_wms.domain.inspection.controller.QualityInspectionController;
...
import com.kb.cosmetic_wms.domain.inspection.exception.*;
...
public class InspectionControllerTest extends RestDocsSupport {
    ...
    class 품질_검사_전표_조회 {
        ...
        void 올바른_전표_ID로_조회하면_200_OK와_상세정보를_반환한다() throws Exception {
            ...
            QualityInspectionDetailResponseDto response = new QualityInspectionDetailResponseDto(
                    inspectionId, InspectionSourceType.INBOUND, 10L, 100L,
                    null, InspectionStatus.WAITING, 20, 0, 0, null);

            given(inspectionService.getInspection(inspectionId)).willReturn(response);
        ...
        void 존재하지_않는_전표_ID로_조회하면_404_NOT_FOUND를_반환한다() throws Exception {
            // given
            Long invalidId = 999L;
            given(inspectionService.getInspection(invalidId)).willThrow(new InspectionNotFoundException());
    ...
    class 품질_검사_시작 {
        ...
        void 올바른_검사자_ID로_시작_요청하면_200_OK와_갱신된_전표를_반환한다() throws Exception {
            // given
            Long inspectionId = 1L;
            InspectionStartRequestDto request = new InspectionStartRequestDto(14L);
            QualityInspectionDetailResponseDto response = new QualityInspectionDetailResponseDto(
                    inspectionId, InspectionSourceType.INBOUND, 10L, 100L,
                    14L, InspectionStatus.IN_PROGRESS, 20, 0, 0, null);

            given(inspectionService.startInspection(eq(inspectionId), eq(14L))).willReturn(response);
        ...
        void 검사자_ID가_누락되면_400_BAD_REQUEST를_반환한다() throws Exception {
            // given
            InspectionStartRequestDto request = new InspectionStartRequestDto(null);
        ...
        void WAITING이_아닌_상태에서_시작_요청하면_409_CONFLICT를_반환한다() throws Exception {
            // given
            InspectionStartRequestDto request = new InspectionStartRequestDto(14L);
            given(inspectionService.startInspection(anyLong(), anyLong()))
    ...
    class 품질_검사_완료 {
        ...
        void 올바른_수량으로_완료_요청하면_200_OK와_완료된_전표를_반환한다() throws Exception {
            ...
            InspectionCompleteRequestDto request = new InspectionCompleteRequestDto(18, 2, "포장 불량");
            QualityInspectionDetailResponseDto response = new QualityInspectionDetailResponseDto(
                    inspectionId, InspectionSourceType.INBOUND, 10L, 100L,
                    14L, InspectionStatus.COMPLETED, 20, 18, 2, "포장 불량");

            given(inspectionService.completeInspection(eq(inspectionId), eq(18), eq(2), eq("포장 불량")))
                    .willReturn(response);
        ...
        void IN_PROGRESS가_아닌_상태에서_완료_요청하면_409_CONFLICT를_반환한다() throws Exception {
            // given
            InspectionCompleteRequestDto request = new InspectionCompleteRequestDto(20, 0, null);
            given(inspectionService.completeInspection(anyLong(), anyInt(), anyInt(), any()))
        ...
        void 수량_합계가_총_검사수량과_불일치하면_422_UNPROCESSABLE_ENTITY를_반환한다() throws Exception {
            // given
            InspectionCompleteRequestDto request = new InspectionCompleteRequestDto(5, 5, null);
            given(inspectionService.completeInspection(anyLong(), anyInt(), anyInt(), any()))
