...
import com.kb.cosmetic_wms.domain.order.controller.OrderController;
...
public class OrderControllerTest extends RestDocsSupport {
    ...
    class 발주_신청 {
        ...
        void 올바른_발주_정보가_주어지면_201_CREATED와_신청된_전표를_반환한다() throws Exception {
            ...
            OrderResponseDto response = pendingOrderResponse(1L);
        ...
        void 발주_품목이_비어있으면_400_BAD_REQUEST와_ORDER_ITEMS_REQUIRED를_반환한다() throws Exception {
            ...
            given(orderService.createOrder(any())).willThrow(new OrderItemsRequiredException());
    ...
    class 발주_확정 {
        ...
        void 발주_신청_상태의_전표를_확정하면_200_OK와_CONFIRMED_상태의_전표를_반환한다() throws Exception {
            // given
            Long orderId = 1L;
            OrderResponseDto response = orderResponse(orderId, OrderStatus.CONFIRMED);
            given(orderService.confirmOrder(orderId)).willReturn(response);
        ...
        void 이미_확정된_전표에_재확정_요청하면_409_CONFLICT를_반환한다() throws Exception {
            // given
            given(orderService.confirmOrder(anyLong())).willThrow(new OrderConfirmNotAllowedException());
        ...
        void 존재하지_않는_발주_ID로_확정_요청하면_404_NOT_FOUND를_반환한다() throws Exception {
            // given
            given(orderService.confirmOrder(anyLong())).willThrow(new OrderNotFoundException());
    ...
    class 배송_준비_시작 {
        ...
        void 발주_확정_상태의_전표에_배송_준비_시작을_요청하면_200_OK와_PREPARING_상태를_반환한다() throws Exception {
            // given
            Long orderId = 1L;
            OrderResponseDto response = orderResponse(orderId, OrderStatus.PREPARING);
            given(orderService.startPreparation(orderId)).willReturn(response);
        ...
        void 발주_확정_이외의_상태에서_배송_준비_시작을_요청하면_409_CONFLICT를_반환한다() throws Exception {
            // given
            given(orderService.startPreparation(anyLong())).willThrow(new OrderPreparationNotAllowedException());
    ...
    class 배송_출하 {
        ...
        void 배송_준비_중_상태의_전표에_출하를_요청하면_200_OK와_SHIPPED_상태를_반환한다() throws Exception {
            // given
            Long orderId = 1L;
            OrderResponseDto response = orderResponse(orderId, OrderStatus.SHIPPED);
            given(orderService.ship(orderId)).willReturn(response);
        ...
        void 배송_준비_중_이외의_상태에서_출하를_요청하면_409_CONFLICT를_반환한다() throws Exception {
            // given
            given(orderService.ship(anyLong())).willThrow(new OrderShipNotAllowedException());
    ...
    class 배송_완료 {
        ...
        void 배송_중_상태의_전표에_배송_완료를_요청하면_200_OK와_DELIVERED_상태를_반환한다() throws Exception {
            // given
            Long orderId = 1L;
            OrderResponseDto response = orderResponse(orderId, OrderStatus.DELIVERED);
            given(orderService.completeDelivery(orderId)).willReturn(response);
        ...
        void 배송_중_이외의_상태에서_배송_완료를_요청하면_409_CONFLICT를_반환한다() throws Exception {
            // given
            given(orderService.completeDelivery(anyLong())).willThrow(new OrderDeliveryCompleteNotAllowedException());
    ...
    class 발주_취소 {
        ...
        void 발주_신청_상태의_전표에_취소를_요청하면_200_OK와_CANCELED_상태를_반환한다() throws Exception {
            // given
            Long orderId = 1L;
            OrderResponseDto response = orderResponse(orderId, OrderStatus.CANCELED);
        ...
        void 발주_확정_이후_상태에서_취소를_요청하면_409_CONFLICT를_반환한다() throws Exception {
            // given
            given(orderService.cancelOrder(anyLong())).willThrow(new OrderCancelNotAllowedException());
        ...
        void 존재하지_않는_발주_ID로_취소_요청하면_404_NOT_FOUND를_반환한다() throws Exception {
            // given
            given(orderService.cancelOrder(anyLong())).willThrow(new OrderNotFoundException());
    ...
    private static OrderResponseDto pendingOrderResponse(Long id) {
        return orderResponse(id, OrderStatus.PENDING);
    }

    private static OrderResponseDto orderResponse(Long id, OrderStatus status) {
        return new OrderResponseDto(
                id, 1L, 10L, status,
                List.of(new OrderResponseDto.OrderItemResponseDto(1L, 1L, 10))
        );
    }
